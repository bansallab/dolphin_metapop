...
for sims in models_reformat:
    ...
    for df in sims:
        ...
        if j == 9:
          ...
          #  plt.savefig("Figures/SS_fit" +str(labels[i])+".jpg", dpi = 300, bbox_inches = 'tight')
            plt.show()
            plt.close()
        j = j +1
...
sseDF_CI.to_csv('SSE_Data_scenarios.csv') 
Corr_DF_CI.to_csv('r_Data_scenarios.csv') 
...
for d in datasets:
    ...
    final_CI_SS_data.to_csv("SS_for_all_models2013" +string_labels[r]+".csv")  
    ...
    if r in range(0,3):
        ax.set_ylabel("2013 r", fontsize =25) 
        ax.set_ylim([0,1])
    else:
        ax.set_ylabel("2013 SSE", fontsize =25) 
        ax.set_ylim([0,10])
    
    plt.savefig("FULL_boxplot2013" + string_labels[r]+ ".jpg", dpi = 300, bbox_inches= 'tight')
    plt.show()
    plt.close()
